export { ArticleDetailsComponent } from './ui/article-details-component/article-details-component';

export type { Article } from './model/types/article';
export type { ArticleDetailsScheme } from './model/types/article-details-scheme';

export { ArticleList } from './ui/article-list/article-list';

export { ArticleView } from './model/types/article';

export { ArticleViewSelector } from './ui/article-view-selector/article-view-selector';

export { ArticleTypeTabs } from './ui/article-type-tabs/article-type-tabs';

export { getArticleDetailsData } from './model/selectors/article-details';
